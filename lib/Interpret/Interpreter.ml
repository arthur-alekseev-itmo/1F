...
module Interpreter = struct
  ...
  let rec interpret_decl ctx (d : decl) =
    ...
    | LetDeclRecursiveGroup decls ->
        let register ctx decl =
          let lazy_value = VLazy (decl.body, ctx.locals, Some decl.name) in
          set_pattern_to_ctx decl.name lazy_value ctx
        in
        let eval ctx decl =
          let body = eval_expr decl.body ctx in
          set_pattern_to_ctx decl.name body ctx
        in
        let ctx = List.fold_left register ctx decls in
        List.fold_left eval ctx decls
    | LetDecl d ->
