...
let () =
  ...
  Printf.printf "\n--- Парсеры (только парсинг, без лексера) ---\n%!";
  ...
  run_bench "Рукописный Parser" iterations (fun () ->
    let _ = OneF.Parsing.Parser.Parser.parse_program tokens in ());
  ...
  Printf.printf "\n--- Полный цикл (lexer + parser сравнение) ---\n%!";
  ...
  run_bench "MenhirParser" small_iter (fun () ->
    ...
    | Error _ -> ());
  run_bench "AngstromParser (лексинг + парсинг за один проход)" small_iter (fun () ->
    match OneF.Parsing.AngstromParser.AngstromParser.program_of_string input with
    | Ok _ -> ()
    | Error _ ->
      match OneF.Parsing.AngstromParser.AngstromParser.expr_of_string input with
