...
let try_parse_program input tokens =
  let m = OneF.Parsing.MenhirDriver.parse_program input tokens in
  match m with
  | Ok _ -> `Prog
  | Error _ ->
...
let parse_hand tokens =
  let open OneF.Parsing.Parser.Parser in
  let _ = parse_program tokens in ()

let parse_menhir_prog input tokens =
  match OneF.Parsing.MenhirDriver.parse_program input tokens with
  | Ok _ -> ()
  | Error _ ->
    let _ = OneF.Parsing.MenhirDriver.parse_expr input tokens in ()

let parse_ocamlyacc_prog input tokens =
  match OneF.Parsing.OcamlyaccDriver.parse_program input tokens with
  | Ok _ -> ()
  | Error _ ->
    let _ = OneF.Parsing.OcamlyaccDriver.parse_expr input tokens in ()

let parse_angstrom input =
  match OneF.Parsing.AngstromParser.AngstromParser.program_of_string input with
  | Ok _ -> ()
  | Error _ ->
    let _ = OneF.Parsing.AngstromParser.AngstromParser.expr_of_string input in ()

let parse_menhir_table_prog input tokens =
  match OneF.Parsing.MenhirDriver.parse_program_table input tokens with
  | Ok _ -> ()
  | Error _ ->
    match OneF.Parsing.MenhirDriver.parse_expr_table input tokens with
    | Ok _ -> ()
    | Error _ -> ()
...
let () =
  ...
  let kind = try_parse_program input tokens in
    ...
    | `Prog ->
      (match OneF.Parsing.MenhirDriver.parse_program input tokens with
       | Ok prog ->
           ...
           f (List.length prog) nodes
       | _ -> ())
    | `Expr ->
      (match OneF.Parsing.MenhirDriver.parse_expr input tokens with
       | Ok expr -> f 1 (count_expr expr)
       | _ -> ())
    | `Error -> ()
  in
  ...
  (match OneF.Parsing.Parser.Parser.parse_program tokens with
   | OneF.Parsing.Parser.Parser.Parsed (prog, _) ->
       let nodes = List.fold_left (fun a d -> a + count_decl d) 0 prog in
       Printf.printf "Рукописный Parser:   %d деклараций, %d узлов AST\n%!"
         (List.length prog) nodes
   | _ -> Printf.printf "Рукописный Parser:   ошибка парсинга\n%!");
  ...
  (match OneF.Parsing.AngstromParser.AngstromParser.program_of_string input with
   | Ok prog ->
       let nodes = List.fold_left (fun a d -> a + count_decl d) 0 prog in
       Printf.printf "AngstromParser:      %d деклараций, %d узлов AST\n%!"
         (List.length prog) nodes
   | Error _ ->
     match OneF.Parsing.AngstromParser.AngstromParser.expr_of_string input with
     | Ok expr ->
         let nodes = count_expr expr in
         Printf.printf "AngstromParser:      1 выражение, %d узлов AST\n%!" nodes
     | Error e -> Printf.printf "AngstromParser:     ошибка: %s\n%!" e);
  ...
  run_bench "Рукописный Parser" iterations (fun () ->
    let _ = parse_hand tokens in ());
  run_bench "MenhirParser" iterations (fun () ->
    parse_menhir_prog input tokens);
  run_bench "OcamlyaccParser" iterations (fun () ->
    parse_ocamlyacc_prog input tokens);
  run_bench "AngstromParser" iterations (fun () ->
    parse_angstrom input);
  run_bench "MenhirParserTable" iterations (fun () ->
    parse_menhir_table_prog input tokens);

  Printf.printf "\n--- Полный цикл (лексинг + парсинг) ---\n%!";
  let small_iter = max 1 (min 100 (10000 / max 1 (String.length input))) in
  run_bench "Рукописный Parser" small_iter (fun () ->
    let _ = OneF.Parsing.Parser.Parser.program_of_string input in ());
  run_bench "MenhirParser" small_iter (fun () ->
    match lex_string input with
    | Ok t -> parse_menhir_prog input t
    | Error _ -> ());
  run_bench "OcamlyaccParser" small_iter (fun () ->
    match lex_string input with
    | Ok t -> parse_ocamlyacc_prog input t
    | Error _ -> ());
  run_bench "AngstromParser (однопроходный)" small_iter (fun () ->
    parse_angstrom input);
  run_bench "MenhirParserTable" small_iter (fun () ->
    match lex_string input with
    | Ok t -> parse_menhir_table_prog input t
    | Error _ -> ())