module Lexemes : sig
  type lexeme_payload =
    ...
    | Let | RecFlag | In (** Let expression, syntax is "пусть <имя> = <выражение> в <выражение>" *)
    | Operator of string (** Operator, that is any combination of characters: +-*/=@.*)
    | Wildcard (** The "_" symbol*) 
    | Identifier of string (** Identifier, i.e. "окэмл_про", "ф1", "_33", starts with А-Я/а-я/_*)
    ...
    | LBr | RBr (** "[", "]" *)
    | LPar | RPar (** "(", ")" *)
    | If | Then | Else (** ITE expression, syntax is "если <выражение> то <выражение> иначе <выражение>" *)
    | VBar
    | Semicolon (** ";" *)
